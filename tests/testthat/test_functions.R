test_that("The league url is pasted correctly", {
  expect_equal(build_league_url(league_url_name = "berlin-kreisklasse-a-staffel-1-6322", "2018-19"), expected = "https://www.kicker.de/berlin-kreisklasse-a-staffel-1-6322/spieltag/2018-19/-1")
})

test_that("Warning, if the team name is wrong", {
  expect_warning(all_game_data(league_data = amateuR::data, team = "This does not make any sense"))
})

test_that("past_games has correct format", {
  expect_equal(ncol(all_game_data(league_data = amateuR::data)$past_games), expected = 4)
})

test_that("past_special_events has correct format", {
  expect_equal(ncol(all_game_data(league_data = amateuR::data)$past_special_events), expected = 3)
})

test_that("future has correct format", {
  expect_equal(ncol(all_game_data(league_data = amateuR::data)$future), expected = 4)
})

test_that("past_games is filled if season is over!", {
  testthat::expect_true(nrow(all_game_data(league_data = amateuR::data)$past_games) > 0)
})

test_that("plot function does not give any output", {
  testthat::expect_null(plot_goal_difference(all_game_data(amateuR::data), team = "TSC Dorste"))
})

test_that("plot function does not give any output", {
  testthat::expect_null(plot_points(all_game_data(amateuR::data), team = "TSC Dorste"))
})
